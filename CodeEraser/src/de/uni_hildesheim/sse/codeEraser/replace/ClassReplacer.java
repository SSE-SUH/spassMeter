...
public class ClassReplacer {
    ...
    public static void main(String[] args) {
        ...
        Option patOption = OptionBuilder.withArgName("id=value").hasArgs(2)
            .withValueSeparator().withDescription("specify name patterns")
            .create("P");
        options.addOption(patOption);
        ...
        try {
            ...
            if (line.hasOption(patOption.getOpt())) {
                Properties props = line.getOptionProperties(patOption.getOpt());
                for (Map.Entry<Object, Object> entry : props.entrySet())  {
                    conf.addPattern(entry.getKey().toString(), 
                        entry.getValue().toString());
                }
            }
